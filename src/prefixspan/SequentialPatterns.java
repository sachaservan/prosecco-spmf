...
public class SequentialPatterns {
	...
	public SequentialPatterns clone() {
		
		int k = 0;
		SequentialPatterns clone = new SequentialPatterns(this.getName());
		for (List<SequentialPattern> level : this.getLevels()) {
			for (SequentialPattern pattern : level) 
				clone.addSequence(pattern.clone(), k);
			k++;
		}
	...
	public SequentialPattern get(SequentialPattern pattern) {
		
		for(SequentialPattern pat : levels.get(pattern.size())) {
