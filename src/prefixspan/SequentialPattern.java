...
public class SequentialPattern implements Comparable<SequentialPattern>{
	...
	public void addSequenceIDs(List<Integer> sequencesIds) {
		this.sequencesIds.addAll(sequencesIds);
	}
	...
	public String getRelativeSupportFormated(int sequencecount) {
		double relSupport = ((double)sequencesIds.size() + additionalSupport) / ((double) sequencecount);
		// pretty formating :
	...
	public int getAbsoluteSupport(){
		return sequencesIds.size() + additionalSupport;
	}
...
//		}
	...
	public SequentialPattern clone() {
	...
	public boolean equals(SequentialPattern other){
		
		if (other.getItemsets().size() != itemsets.size()) {
			return false;
		}
		
		
		for(int i = 0; i < itemsets.size(); i++) {
			if (other.getItemsets().get(i).size() != itemsets.get(i).size()) {
				return false;
			}
			
			if (!other.getItemsets().get(i).containsAll(itemsets.get(i))) {
				return false;
			}
